...
|-------|--------------------------------------------------------------------------------|-------------------|------------------|-------------------------------------------------------|
| 1     | Развертывание Kafka-кластера, настройка топиков для транзакционных уведомлений | 2 недели          | 1 SRE, 1 backend | Задержки с настройкой инфраструктуры                  |
| 2     | Разработка Notification API (прием уведомлений, валидация, публикация в Kafka) | 2 недели          | 2 backend        | Интеграция с существующими сервисами-отправителями    |
| 3     | Разработка Delivery Orchestrator (state machine, failover, дедупликация)       | 3 недели          | 2 backend        | Сложность логики state machine, корректность таймеров |
| 4     | Разработка Channel Workers (push, SMS, email) + интеграция с провайдерами      | 2 недели          | 2 backend        | Нестабильность внешних провайдеров при тестировании   |
| 5     | Мониторинг, алертинг, нагрузочное тестирование, запуск в production            | 1-2 недели        | 1 SRE, 1 backend | Непредвиденные проблемы под нагрузкой                 |
...
| Этап  | Описание                                                                       | Планируемый срок   | Ресурсы                   | Риски                                           |
|-------|--------------------------------------------------------------------------------|--------------------|---------------------------|-------------------------------------------------|
| 1     | Развертывание Kafka-кластера, настройка топиков по каналам + Redis Cluster     | 2 недели           | 1-2 SRE, 1 backend        | Больше инфраструктуры, чем в Варианте 1         |
| 2     | Разработка Notification API (прием, валидация, построение цепочки, публикация) | 2 недели           | 2 backend                 | Корректность построения цепочки из предпочтений |
| 3     | Разработка Channel Services (push, SMS, email) - по шаблону, независимо        | 3 недели           | 3 backend (по 1 на канал) | Согласованность контрактов между сервисами      |
| 4     | Разработка Failover Router (маршрутизация по результатам, запись в Redis)      | 2 недели           | 1-2 backend               | Корректность дедупликации и обработки гонок     |
| 5     | Мониторинг, distributed tracing, нагрузочное тестирование, запуск в production | 2 недели           | 1 SRE, 1 backend          | Сложность отладки распределенного потока        |
